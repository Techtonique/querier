...
def parse_request(x):
    ...
    str_conds = "(df['"+left_exps[0]+"'] "+ ops_conds[0] + " " + right_exps[0]+ ")"        
    idx_cond = 1
    
    while (idx_cond < n_conds):
        try:
            op = ops_between_conds[idx_cond-1]
            str_conds += " " + op +" (df['"+left_exps[idx_cond]+"']"+ " "+ ops_conds[idx_cond]+ " " + right_exps[idx_cond]+ ")"
            idx_cond += 1 
        except:
            raise ValueError('invalid request: check column names, column contents, operators and parentheses')
            return
...
def parse_update_request(x):
    ...
    if (idx_par_begin > 0) & (z.find("(") > 0): # e.g np.mean(tip)
        ...
        if (key1 == key2):                        
            ...
            return "df['"+ key1 +"']" + " = "  + z.replace(key2, "df['" + key2 + "']")
    ...
    if (key1.isalpha() & key2.isalpha()):
        ...
        if key2.isalpha() == False:
            ...
            key3 = temp[-1]   
            op = temp[1]
            return "df['"+ key1 +"']" + " = " + key2 + op + "df['" + key3 + "']"
...
def parse_cols_request(x): 
    ...
    if cols_text.find('as') < 0:
        for idx, elt in enumerate(cols_text_split):
            idx_as = elt.find("(")
            if idx_as > 0:
                cols_text_split[idx] = elt.replace(" ", "").replace("(", "_").replace(")", "").replace("*", "all").lower()
        
        return [cols_text_split[idx].replace(" ", "") for idx in range(len(cols_text_split))]
